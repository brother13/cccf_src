<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.keyword" clearable placeholder="被控制对象、案号" style="width: 220px"
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        rpalist(this.listQuery).then((response) => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          ...
          if (valid) {
            rpalistupdate(newtemp).then((response) => {
              // console.log(data);
              const data = response
              if (data.code === 20000) {
                this.dialogFormVisible = false
      ...
      handleDelete(row) {
        ...
        this.$confirm('此操作将删除该记录, 是否继续?', '提示', {
          ...
          rpalistdel(row.rpalistid).then((response) => {
            const data = response
            if (data.code === 20000) {
              this.$notify({
                title: '操作完成',
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then((excel) => {
          const tHeader = ['办案人','案号','申请人','被申请人','财产类型','首封状态','开始日期','届满日期','财产情况','备注','状态']
          const filterVal = ['cbr','ah','sqzxr','bzxr','type','cfsf','startdate','enddate','ccqk','note','isvoid']
