      ...
      >
        ...
        <el-form-item label="仅作文件下载">
          <el-switch
            ...
            :inactive-value="0"
          />
          <el-tag>{{ temp.onlyfile == 1 ? '仅作文件下载' : '普通文章' }}</el-tag>
        </el-form-item>

        <el-form-item v-if="temp.onlyfile==0" label="是否是链接">
          <el-switch
            v-model="temp.islink"
            active-color="#13ce66"
            inactive-color="#ff4949"
        ...
        <template v-else>
          ...
          <el-form-item v-if="!temp.onlyfile" label="内容" prop="content">
            <Tinymce
              ref="editor"
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          // 判断是否是仅文件上传，如果是仅文件下载，则不允许附件为空
          if (this.temp.onlyfile) {
            if (this.temp.attachment.length < 1) {
              this.$alert('您已选择仅文件下载，附件不得为空！')
              return false
            }
          }
          this.temp.typeid = this.pageconfig.typeid
    ...
    changeCatalog(id) {
      // console.log('changeCatalog', id)

      for (let i = 0; i < this.CatalogList.length; i++) {
        const row = this.CatalogList[i]
        if (row.id === id) {
          // 判断是否是仅文件的
          if (row.onlyfile === 1) {
            this.temp.onlyfile = 1
          } else {
            this.temp.onlyfile = 0
          }
        }
      }
    },
    ...
    handleSuccess(response, file, fileList) {
      ...
      const filename = fileinfo.oldname
      if (this.temp.newstitle === '') {
        this.temp.newstitle = filename
      }
      if (this.temp.description === '') {
        this.temp.description = filename
      }
