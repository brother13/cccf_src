<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-date-picker label="起始日期" v-model="listQuery.startdate" type="date" placeholder="起始日期" class="filter-item"
        style="width: 150px" value-format="yyyy-MM-dd" @change="handleFilter" />
      <el-date-picker label="届满日期" v-model="listQuery.enddate" type="date" placeholder="届满日期" class="filter-item"
        style="width: 150px;" value-format="yyyy-MM-dd" @change="handleFilter" />

      <el-select v-model="listQuery.deptcode" placeholder="请选择部门" clearable style="width:0px" multiple
        class="filter-item" @change="handleFilter">
        <el-option v-for="item in DeptList" :key="item.deptid" :label="item.fullname"
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"
      ...
      <el-table-column label="添加时间" prop="inserttime" align="center" />
      ...
      <!--      <el-table-column label="查封状态" prop="status" align="center" /> -->
      ...
      <el-table-column min-width="229" label="操作" align="center">
        <template slot-scope="{ row }">
          <el-dropdown split-button=true type="primary" icon="el-icon-edit" @click="handleUpdate(row)">
            <i class="el-icon-edit"></i>编辑
            <el-dropdown-menu>
              <el-dropdown-item icon="el-icon-edit"  @click.native="handleUpdate(row)">编辑</el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete"  @click.native="handleDelete(row)">删除</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          <el-button style="margin-left:5px;margin-top:1px;" type="warning" icon="el-icon-download" @click="downLoadWord(row)">文书</el-button>
...
<!--          <el-button type="primary" size="mini" icon="el-icon-edit" @click="handleUpdate(row)">编辑</el-button>
          ...
          <el-button size="mini" type="danger" click2="handleModifyStatus(row,'deleted')" icon="el-icon-delete"
          ...
          <el-col span="12">
            <el-form-item label="财产类型" prop="type">
              <el-select @change="handleDateChange()" v-model="temp.type" style="width: 100%" class="filter-item"
                ...
                <el-option v-for="item in Cftype" :key="item.id" :label="item.typename"
                  :value="item.typename">{{ item.typename }}</el-option>
              </el-select>
            </el-form-item>
          </el-col><el-col span="12">
            <el-form-item label="首封状态" prop="cfsf">
              <el-select @change="handleCfsfChange()" v-model="temp.cfsf" style="width: 100%" class="filter-item"
                placeholder="请选择" clearable>
                <el-option v-for="item in cfsf" :key="item.id" :label="item.cfsf"
                  :value="item.cfsf">{{ item.cfsf }}</el-option>
              </el-select>
            </el-form-item>
          </el-col><el-col span="12">
            ...
            <el-form-item label="控制类型">
              <el-select v-model="temp.leixing" style="width: 100%" class="filter-item" placeholder="请选择" clearable>
                <el-option v-for="item in Leixing" :key="item.id" :label="item.name"
    ...
    data() {
      return {
        ...
        listQuery: {
          page: 1,
          pagesize: 10,
          startdate: parseTime(new Date().getTime() - 7 * 24 * 60 * 60 * 1000, '{y}-{m}-{d}'),
