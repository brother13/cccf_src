<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="案号/当事人" prop="ah" align="center" width="200">
        <template slot-scope="{row}">
          <span>{{ row.ah }}</span>
          <br>
          <span>承办人：{{ row.cbr }}</span>
          <br>
          <span>{{ row.bzxr }}</span>
          <br>
          <span>{{ row.zjhm }}</span>
      ...
      <el-table-column label="公司信息" align="center" width="250">
        <template slot-scope="{row}">
          <span>{{ row.qymc }}</span>
          <br>
          <span>{{ row.tyshxydm }}</span>
          <br>
          <span>地址：{{ row.qydz }}</span>
        </template>
      </el-table-column>
      <el-table-column label="公司状态" align="center" width="250">
        <template slot-scope="{row}">
          <span>{{ row.qylx }}</span>

          <br>
          <span>期限：{{ row.jyqxs }}&nbsp;~&nbsp;{{ row.jyqxz }}</span>
          <br>
          <span>状态：{{ row.qyzt }}</span>
          <br>
          <span>监管：{{ row.qydjjg }}</span>
        </template>
      </el-table-column>
      <el-table-column label="经营范围" align="center" width="250">
        <template slot-scope="{row}">
          <span>{{ row.jyfw }}</span>
        </template>
      </el-table-column>
      <el-table-column label="检查时间" prop="fksj" align="center" width="200">
        <template slot-scope="{row}">
          <span>反馈：{{ row.fksj }}</span>
          <br>
          <span>检测：{{ row.updateTime ? row.updateTime : row.createTime }}</span>
        </template>
      </el-table-column>
