<template>
  <div v-bind="$attrs" style="display: inline" v-on="$listeners">
    ...
    <el-dialog v-dialogDrag :visible.sync="showWindow" title="导入通知书" width="80%">
          ...
          >
            <template slot-scope="{ row }">
              <el-button
                ...
                @click="importNotice(row.noticenum)"
              >
                导入</el-button>
            </template>
          </el-table-column>
...
const fieldList = [
  ...
  { field: 'frombill', label: '款项来源', export: true, show: true, width: 120 },

  { field: 'dwname', label: '缴款单位', export: true, show: true, width: 180 },
  { field: 'je', label: '金额', export: true, show: true, align: 'right' },
  { field: 'deptname', label: '承办部门', export: true, show: true },
  { field: 'cbr', label: '承办人', export: true, show: true },
  { field: 'opertime', label: '制单日期', export: true, show: true },
  { field: 'bankname', label: '缴款银行', export: true, show: true },
  { field: 'bankaccount', label: '缴款账号', export: true, show: true },
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      caseapi.notice.getList(this.listQuery).then((res) => {
