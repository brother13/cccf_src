<template>
  <el-dialog v-dialogDrag v-loading="loading" title="绑定诉转诉" :visible.sync="showWindow" :close-on-click-modal="false"
    width="90%">
    <el-row :gutter="20">
              ...
              <el-table key="ssftemp" v-loading="listLoading" :data="tableData" border fit highlight-current-row
                ...
                <el-table-column width="100" align="center" label="操作" >
                  <template slot-scope="{ row }">
                    <el-button-group v-if="!row.used">
                      <el-button size="mini" @click="bindCasetk(row)" type="warning">绑定</el-button>
                    </el-button-group>
                  </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async bindCasetk(row){
      const res = await this.$confirm("您确定要绑定回单至当前退款记录吗？").catch((e)=>{
      ...
      const res2 = await caseapi.bankpdf.bindTkInfoById(query).catch((e)=>{
        this.$alert("绑定失败！"+e.message);
      });
      if(!res2){
        return false;
      }
    ...
    getList() {
      ...
      caseapi.bankpdf.querySsftkList(this.listQuery).then((res) => {
