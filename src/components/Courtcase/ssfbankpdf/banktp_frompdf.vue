<template>
  <el-dialog v-dialogDrag v-loading="loading" title="绑定退票" :visible.sync="showWindow" :close-on-click-modal="false"
    width="90%">
    <el-row :gutter="20">
      ...
      <el-col :span="12">
        ...
        <div>
          ...
          <el-tabs v-model="tabCode" type="card" @tab-click="handleTabClick">
            <el-tab-pane label="明细记录" name="tklist">
              ...
              <div class="filter-container">
                ...
                <el-select v-model="listQuery.tpstatus" style="width:200px;" @change="handleFilter">
                  <el-option v-for="(item, index) in basedata.tpstatusList" :key="index" :value="item.value"
                ...
                <template v-if="canShowMerge"><span class="form-input">是否合并：</span>
                  <el-select v-model="listQuery.ismerge" style="width:200px;" @change="handleFilter">
                    <el-option v-for="(item, index) in basedata.ismergeList" :key="index" :value="item.value"
                      :label="item.label"></el-option>
                  </el-select></template>
              </div>
              <div v-show="checkedInfo.num">
                ...
                <el-button type="primary" size="mini" @click="bindCasetkTp_batch"
                  :disabled="detail.je != checkedInfo.je">合并退票</el-button>
              </div>
              <el-table key="ssftemp" v-loading="listLoading" :data="tableData" border fit highlight-current-row
                style="width: 100%" height="250" size="mini" @selection-change="changeCheck">
                <el-table-column type="selection" label="选择" :selectable="checkSelectable" v-if="listQuery.ismerge" />
                ...
                <el-table-column width="100" align="center" label="操作" >
                  <template slot-scope="{ row }">
                    <el-button-group v-if="!listQuery.ismerge">
                      <el-button size="mini" @click="bindCasetkTp(row)" type="warning">绑定</el-button>
                    </el-button-group>
                  </template>
                </el-table-column>
            ...
            <el-tab-pane label="其它信息" name="tkform" v-if="tpdetail.id">
              <el-form ref="dataForm" :model="tpdetail" label-position="right" label-width="100px"
                ...
                <el-form-item label="退票日期" prop="tpdate">
                  <el-date-picker v-model="tpdetail.tpdate" type="date" placeholder="退票日期" style="width: 100%"
                    value-format="yyyy-MM-dd" :disabled="tpdetail.tpstatus==1" />
                </el-form-item>

                <el-form-item label="退票理由" prop="tpreason">
                  <el-input v-model="tpdetail.tpreason" :autosize="{ minRows: 2, maxRows: 4 }" type="textarea"
                    placeholder="退票理由" :disabled="tpdetail.tpstatus==1" />
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async doTP() {
      ...
      // 以下开始执行退票操作
      let query = Object.assign({},this.tpdetail);
      if(query.ismerge){
