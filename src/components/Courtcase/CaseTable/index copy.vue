<template>
  <div v-bind="$attrs" style="display: inline" v-on="$listeners">
    ...
    <pagination
      v-if="tableData && tableData.total.num && config && config.doublePageNav"
      v-show="tableData.total.num > 0"
      :total="tableData.total.num"
...
export default {
  ...
  methods: {
    ...
    async exportTable(param, func) {
      ...
      const total = this.tableData.total.num || 0
      ...
      if (total <= pagemax) {
        ...
        this.exportstatus.total = this.tableData.total.num
        this.exportstatus.done = 0
        this.exportstatus.page = 1
        this.exportstatus.totalpage = Math.floor(this.tableData.total.num / this.exportstatus.pagesize)
