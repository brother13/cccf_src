  ...
  <div v-bind="$attrs" style="display: inline" v-on="$listeners">
    ...
    <el-dialog v-dialogDrag :visible.sync="showWindow" title="高级搜索" :close-on-click-modal="false">
      <el-form ref="dataForm" :model="temp" label-position="right" label-width="120px"
        ...
        <el-form-item :label="temp.typeid > 200 && temp.typeid < 300 ? '原单据号' : '单据编号'
          ">
        ...
        <el-form-item label="制单日期">
          ...
          <el-date-picker v-model="temp.starttime" type="date" placeholder="起始日期" style="width: 180px"
            value-format="yyyy-MM-dd" />
          ~
          <el-date-picker v-model="temp.endtime" type="date" placeholder="截止日期" style="width: 180px"
            value-format="yyyy-MM-dd" />
        </el-form-item>
        <el-form-item label="入账时间">

          <el-date-picker v-model="temp.bankdate_start" type="date" placeholder="起始日期" style="width: 200px"
            value-format="yyyy-MM-dd" />
          ~
          <el-date-picker v-model="temp.bankdate_end" type="date" placeholder="截止日期" style="width: 200px"
            value-format="yyyy-MM-dd" />
        </el-form-item>
        ...
        <template v-if="temp.typeid < 300 && temp.typeid > 100">
          ...
          <el-form-item label="案件字号">
            <el-select v-model="temp.casetype" multiple class="form-item">
              <el-option v-for="item in basedata.casetypeList" :key="item.id" :label="item.casetypename"
                :value="item.id" />
            </el-select>
          </el-form-item>
        </template>
        ...
        <el-form-item label="承办部门">
          <el-select v-model="temp.deptname" multiple class="form-item">
            <el-option v-for="item in basedata.deptList" :key="item.deptid" :label="item.deptname"
              :value="item.deptname" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="操作人员">
          ...
          <el-select v-model="temp.username" multiple class="form-item">
            <el-option v-for="item in basedata.userList" :key="item.userid" :value="item.username"
              :label="item.username" />
          </el-select>
        </el-form-item>
        ...
        <template v-if="[301, 302, 303].indexOf(temp.typeid) > -1">
          ...
          <el-form-item label="汇款方全称" v-if="temp.typeid != 303">
        ...
        <el-form-item v-if="temp.typeid == 104 || temp.typeid == 110" label="入库状态">
          <el-select v-model="temp.fsstatus" class="form-item">
            <el-option v-for="item in basedata.fsstatusList" :key="item.value" :label="item.label"
              :value="item.value" />
          </el-select>
        </el-form-item>
      </el-form>
