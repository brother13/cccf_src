<template>
  <div v-bind="$attrs" style="display: inline" v-on="$listeners">
    <el-dialog v-dialogDrag :visible.sync="showWindow" title="未发还理由" :close-on-click-modal="false" width="1000px">
      <el-form ref="dataForm" :rules="rules" :model="detail" label-position="right" label-width="100px"
        style="width: 90%; margin-left: 50px">
        <!-- 批量模式下的情况 -->
        <el-table :data="batchinfo.data" border fit highlight-current-row style="width: 100%" size="mini">
        ...
        <el-form-item label="未发还理由" prop="reason">
          <el-select style="width: 100%" v-model="detail.reason">
            <template>
              <el-option v-for="(item, index) in basedata.reasonList" :label="item.classname" :value="item.classname"
                v-if="item.isvoid == 0" :key="index"></el-option>
            </template>
          </el-select>
        </el-form-item>
        <el-form-item label="具体理由" prop="note">
          <el-input v-model="detail.note" type="textarea" maxlength="50" minlength="2" :rows="3"
            show-word-limit></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="showWindow = false"> 取消 </el-button>
        <el-button type="primary" icon="el-icon-edit" @click="doSave" :disabled="batchinfo.isDoing">
          提交
        </el-button>
      </div>
    </el-dialog>
...
const fieldList = [
  ...
  { field: "reason_reason", label: "未发还理由", width: 130, align: "center", show: true },
  {
    field: "reason_note",
    label: "具体理由",
    width: 180,
