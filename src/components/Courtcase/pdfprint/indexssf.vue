<template>
  <el-dialog v-dialogDrag v-loading="pdfinfo.loading" title="打印预览" :visible.sync="showWindow"
    :close-on-click-modal="false" width="90%" :element-loading-text="'处理中 (' + pdfinfo.done + '/' + pdfinfo.total + ')'"
    ...
    <el-option v-for="(item,index) in printtypeList" :key="index" :value="item.value" :label="item.label"></el-option>
    </el-select>
  
      <el-button icon="el-icon-close" :disabled="pdfinfo.loading" @click="showWindow = false" class="form-input">
        取消
      </el-button>
      <el-button v-if="cancheck" type="primary" icon="el-icon-printer" :disabled="pdfinfo.loading"
        @click="saveAndPrint">
        打印并签收
      </el-button>
