<template>
  <div v-bind="$attrs" style="display: inline" v-on="$listeners">
    <el-button @click="showWin" type="success" icon="el-icon-setting" class="filter-item">单据设置</el-button>
      ...
      <el-form ref="dataForm" label-position="left" label-width="150px" style="width: 600px; margin-left: 50px">
        ...
        <el-form-item label="是否允许缴款通知书">
          <el-select v-model="casesk_onlykptz" style="width:100%;" placeholder="是否允许缴款通知书">
            <el-option v-for="(item, index) in noticeList" :key="index" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getConfig() {
      ...
      let config = caseapi.store.get(this.key);
      if (config) {
