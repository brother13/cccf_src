        ...
        >
          <template slot-scope="{ row }">
            <el-button
              ...
              @click="linkcase( row.ah,row.cbrxm)"
            >
              关联</el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      caseapi.casesk.casebill_getbanklist(this.listQuery).then((data) => {
        // console.log(data)
    ...
    // 关联案号
    linkcase(caseinfo, cbr) {
      const data = { id: this.listQuery.id, caseinfo: caseinfo, cbr: cbr }

      const str = '您是否要将单据【' + this.info.billno + '】的关联案号调整为【' + caseinfo + '】，承办人调整为【' + cbr + '】？'
      ...
      this.$confirm(str, '提示', {
        ...
        caseapi.casesk.updateCasebill_caseinfo(data).then((res) => {
          if (res.code === 20000) {
            this.$alert('关联完成！')
          } else {
