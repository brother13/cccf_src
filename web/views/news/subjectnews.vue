...
const PAGECONFIG = {
  pagename: '主题专栏', // 标题
  pagecode: 'SubjectNews', // 页面名称
  typeid: 5, // 1-普通内容,2-部门动态,3-图片新闻,4-通知公告,5-主题专栏
  catalogname: '专栏', // 标题名称，2为 部门，其它为栏目
  showthumb: true // 是否显示缩略图
}

export default {
  name: PAGECONFIG.pagecode + 'Table',
  components: { Pagination, Filetype },
  directives: { waves },
  filters: {
    statusFilter(status) {
  ...
  methods: {
    ...
    async getCataTree() {
      // 获取所有的catalog
      if (this.pageconfig.typeid === 3 || this.pageconfig.typeid === 1 || this.pageconfig.typeid === 4) {
      ...
      if (this.pageconfig.typeid === 5) {
        // 如果是数据取专题
        const res = await postdata(ACTION.subjectList)

        const subjectList = res.data.items
        for (let i = 0; i < subjectList.length; i++) {
          subjectList[i]['catalogname'] = subjectList[i]['subjectname']
          subjectList[i]['fullname'] = subjectList[i]['subjectname']
        }
        this.CatalogList = subjectList
      }
    },
    // 获取部门列表
    async getDeptList() {
