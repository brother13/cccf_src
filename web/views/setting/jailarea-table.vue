<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="所属监狱" prop="jail" align="center" width="200">
        <template slot-scope="{row}">
          {{ row.jail ? row.jail : '所有监狱' }}
        </template>

      </el-table-column>
      <el-table-column label="监狱区域" prop="jailarea" align="center" width="200" />
      <el-table-column label="备注" prop="note" align="center" width="200" />
      <el-table-column label="排序" prop="rank" align="center" width="100" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="区域名称" prop="jailarea">
          <el-input v-model="temp.jailarea" />
        </el-form-item>

        <el-form-item label="所属监狱" prop="jailname">
          <el-select
            v-model="temp.jail"
            placeholder="所属监狱，不选为所有可见"
            clearable
            style="width: 100%"
            class="filter-item"
          >
...
export default {
  ...
  methods: {
    init() {
      // 获取监狱列表
      postdata('/jail/list', { pagesize: 999 }).then(res => {
        this.jailList = res.data.items
      })
    },
    getList() {
