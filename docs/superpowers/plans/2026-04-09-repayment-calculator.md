...
**Goal:** 改造现有的还款计划计算页面，专门用于计算法院判决执行案件中，被执行人未按时还款或中途还款后的剩余本金和利息。

**Architecture:** 复用现有的利率数据（LPR和基准利率），完全在前端计算。将UI分为输入表单、中途还款管理、计算结果展示三个主要区域。计算逻辑按时间段分割，支持利率自动分段和还款节点处理。

**Tech Stack:** Vue 2, Element UI, JavaScript (ES6)

**设计文档:** [docs/superpowers/specs/2026-04-09-repayment-calculator-design.md](../specs/2026-04-09-repayment-calculator-design.md)
...
| 文件 | 职责 | 操作 |
|------|------|------|
| `src/dagl/views/lixijs/repayment.vue` | 主页面组件，包含表单、计算逻辑、结果展示 | 修改（完全重写） |
| `src/dagl/views/lixijs/calculator.vue` | 提供LPR和基准利率数据参考 | 读取参考 |
...
**文件:**
- 读取: `src/dagl/views/lixijs/calculator.vue`
- 修改: `src/dagl/views/lixijs/repayment.vue`

**说明:** 从 calculator.vue 复制 LPR 和基准利率数据到 repayment.vue

- [ ] **Step 1: 读取 calculator.vue 中的利率数据**

查看 `calculator.vue` 中以下数据：
- `lprData` 数组（约第 313-382 行）
- `benchmarkData` 数组（约第 385-401 行）

- [ ] **Step 2: 将利率数据添加到 repayment.vue**

在 `repayment.vue` 的 `<script>` 标签内、export default 之前添加：
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): add LPR and benchmark rate data"
```

---

## Task 2: 重写 Vue 组件结构（模板部分）

**文件:**
- 修改: `src/dagl/views/lixijs/repayment.vue`

**说明:** 完全重写模板部分，保留页面基本结构和样式
    ...
    <el-card class="box-card">
      ...
      <el-form ref="form" :model="form" label-width="160px">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12" v-if="form.rateSourceType === 'custom'">
        ...
        <el-row :gutter="20" v-if="form.rateSourceType === 'auto'">
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="24">
            <el-button type="primary" size="small" @click="addRepayment" style="margin-bottom: 15px;">
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): rewrite template for execution case calculator"
```

---

## Task 3: 实现 data 和计算逻辑
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): add data and helper methods"
```

---

## Task 4: 实现时间段分割和计算核心逻辑
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): implement period splitting and calculation logic"
```

---

## Task 5: 实现中途还款管理和表单重置
...
resetForm() {
  ...
  this.form.rateSourceType = 'auto'
  this.form.repaymentType = 'interest_first'
  this.form.paidAmount = 0
  this.form.calcDelayInterest = true
  this.form.lprLevel = '1y'
  this.form.endDate = this.formatDate(new Date())
  this.repayments = []
  this.result = null
}
```
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): add repayment management and reset methods"
```
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): add export report function"
```
...
- [ ] **Step 1: 在 style 标签中添加分段表格样式**

在现有的 `<style lang="scss" scoped>` 中添加：
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "feat(repayment): add segment table styles"
```
...
```bash
npm run lint -- --fix src/dagl/views/lixijs/repayment.vue
```
...
浏览器访问 `http://localhost:9527/#/lixijs/repayment`，验证以下功能：
1. 表单正常显示
2. 可以添加/删除中途还款记录
3. 选择日期后可以正常计算
4. 结果展示正常（包括分段明细和还款节点）
5. 导出报告功能正常
...
```bash
git add src/dagl/views/lixijs/repayment.vue
git commit -m "fix(repayment): lint fixes and minor adjustments"
```
...
**1. Spec Coverage:**
- ✅ 债务金额输入
- ✅ 利率类型（基准利率与LPR自动分段、自定义）
- ✅ 还款类型（先息后本、先本后息）
- ✅ 正常履行金额
- ✅ 日期设置（起算日期、延迟履行日期、结束日期）
- ✅ 计算类型（一般利息、加倍利息）
- ✅ 中途还款管理（添加、删除、多笔）
- ✅ 利率自动分段（2020-08-20分界）
- ✅ 迟延履行金最后统一结算
...
**Plan complete and saved to `docs/superpowers/plans/2026-04-09-repayment-calculator.md`. Two execution options:**
